...
def _n_filters(count: int) -> list[dict]:
    """`count` distinct standard-ID accept rules (0x100 + i stays < 0x7FF)."""
    return [{"can_id": 0x100 + i} for i in range(count)]


def test_route_filters_max_accepted(set_core_config) -> None:
    # 255 is the largest count that survives the trip through the C++
    # GatewayRoute(..., uint8_t rule_count, ...) argument intact.
    setup_c6(set_core_config)
    validated = validate(gateway(routes=[route(filters=_n_filters(255))]))
    assert len(validated["routes"][0]["filters"]) == 255


def test_route_filters_over_max_rejected(set_core_config) -> None:
    # 256 would narrow to uint8_t 0 in C++: every rule silently dropped, the
    # route degraded to its bare default_action. With `default_action: accept`
    # an allow-list route would then forward everything onto the bus.
    setup_c6(set_core_config)
    with pytest.raises(cv.Invalid):
        validate(gateway(routes=[route(filters=_n_filters(256))]))


def test_route_filters_empty_rejected(set_core_config) -> None:
    # The lower bound of the same cv.Length: `filters: []` is a user mistake.
    setup_c6(set_core_config)
    with pytest.raises(cv.Invalid):
        validate(gateway(routes=[route(filters=[])]))
