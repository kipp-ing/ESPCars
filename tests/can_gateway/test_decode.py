"""Signal-decode schema tests (cfg-decode, cfg-v19/v20/v25/v29).
...
def test_sna_absent_by_default(set_core_config) -> None:
    # No `sna:` key means no SNA value exists for the signal — the pre-SNA
    # behavior, where every raw value is a reading.
    _decode_gateway(set_core_config)
    validated = _sensor(_decode(offset=0, length=1))
    assert "sna" not in validated


def test_sna_all_ones_accepted(set_core_config) -> None:
    _decode_gateway(set_core_config)
    validated = _sensor(_decode(bit_offset=0, bit_length=10, sna="all_ones"))
    assert validated["sna"] == "all_ones"
    # Case-insensitive, like the other literal-valued decode keys (byte_order).
    assert _sensor(_decode(offset=0, length=1, sna="ALL_ONES"))["sna"] == "all_ones"


def test_sna_explicit_value_accepted(set_core_config) -> None:
    _decode_gateway(set_core_config)
    # The escape hatch: a DBC reserving something other than all-ones. 0x80 in
    # a signed byte is the raw pattern, not the -128 it sign-extends to.
    validated = _sensor(_decode(offset=0, length=1, signed=True, sna=0x80))
    assert validated["sna"] == 0x80
    # 0 is a legal marker and must not read as "no sna configured".
    assert _sensor(_decode(offset=0, length=1, sna=0))["sna"] == 0
...
def test_v29_sna_wider_than_a_bit_signal_rejected(set_core_config) -> None:
    _decode_gateway(set_core_config)
    # 1023 is the widest raw value a 10-bit field can carry; 1024 never can, so
    # the marker would silently never match.
    with pytest.raises(cv.Invalid, match="does not fit this 10-bit signal"):
        _sensor(_decode(bit_offset=0, bit_length=10, sna=1024))
    _sensor(_decode(bit_offset=0, bit_length=10, sna=1023))


def test_v29_sna_wider_than_a_byte_signal_rejected(set_core_config) -> None:
    _decode_gateway(set_core_config)
    # Byte form: the width is 8 bits per byte of `length`.
    with pytest.raises(cv.Invalid, match="does not fit this 8-bit signal"):
        _sensor(_decode(offset=0, length=1, sna=0x100))
    with pytest.raises(cv.Invalid, match="does not fit this 16-bit signal"):
        _sensor(_decode(offset=0, length=2, sna=0x10000))
    _sensor(_decode(offset=0, length=2, sna=0xFFFF))


def test_v29_sna_all_ones_never_needs_a_width_check(set_core_config) -> None:
    # The literal is resolved at runtime from the signal's own width, so it is
    # valid on every width including the 32-bit edge.
    _decode_gateway(set_core_config)
    assert _sensor(_decode(offset=0, length=4, sna="all_ones"))["sna"] == "all_ones"
    assert _sensor(_decode(bit_offset=0, bit_length=1, sna="all_ones"))["sna"] == (
        "all_ones"
    )


def test_v29_sna_bad_literal_rejected(set_core_config) -> None:
    _decode_gateway(set_core_config)
    with pytest.raises(cv.Invalid, match="all_ones"):
        _sensor(_decode(offset=0, length=1, sna="all_one"))


def test_v29_sna_negative_rejected(set_core_config) -> None:
    _decode_gateway(set_core_config)
    with pytest.raises(cv.Invalid):
        _sensor(_decode(offset=0, length=1, sna=-1))


def test_v29_sna_boolean_rejected(set_core_config) -> None:
    # `sna: true` reads like a flag but a bool is an int in Python, so without
    # the guard it would configure the raw value 1.
    _decode_gateway(set_core_config)
    with pytest.raises(cv.Invalid, match="not a flag"):
        _sensor(_decode(offset=0, length=1, sna=True))
