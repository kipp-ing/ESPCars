    ...
    sensor.sensor_schema(CanGatewayDecodeSensor).extend(
        {
            **decode_source_schema(),
            ...
            # Opt-in "signal not available" handling: `all_ones` for the DBC
            # convention (all ones for the signal's own width), or the raw
            # value a DBC reserves instead. Absent means no SNA value exists —
            # every raw value stays a reading, exactly as before this key.
            cv.Optional(CONF_SNA): validate_sna_value,
        }
    ),
    validate_decode_id,
    validate_signal_position,
...
async def _decode_to_code(config):
    ...
    if (sna := config.get(CONF_SNA)) is not None:
        # `all_ones` stays symbolic down to the runtime, which derives it from
        # the signal width — one source of truth for the width -> marker map
        # (width_all_ones() in gateway_core.h), not a copy computed here.
        if sna == SNA_ALL_ONES:
            cg.add(var.set_sna(SnaMode.ALL_ONES, 0))
        else:
            cg.add(var.set_sna(SnaMode.EXPLICIT, sna))
    cg.add(
