 ...
 public:
  ...
  bool read_latest(FrameSnapshot &out) {
    for (uint8_t attempt = 0; attempt < 3; attempt++) {
      ...
      // Seqlock reader fence. The acquire on seq_before keeps the copy from being hoisted above it,
      // but an acquire load orders only what FOLLOWS it — without this fence the non-atomic copy may
      // sink below the seq_after load (compiler or weakly-ordered CPU), and a torn record would then
      // pass validation. Reproduced on arm64 by tests/host/test_snapshot.cpp.
      std::atomic_thread_fence(std::memory_order_acquire);
      uint32_t seq_after = entry.seq.load(std::memory_order_relaxed);
...
/// One captured frame for a datalogger tap (sd_logger spec D1). FrameRecord carries what an
/// application needs to *act* on a frame; a log needs two things it deliberately omits: when the
/// frame arrived, and which port saw it. The timestamp is taken in the RX ISR at capture time, not
/// when the writer task gets round to it, so SD write-latency spikes shift nothing in the log.
///
/// 20 bytes, and the static_assert holds it there: at the design load (90% of 500 kbps on two
/// segments, ~7200 frames/s) every byte is ~7 KB/s of ring bandwidth.
struct TapRecord {
  uint32_t t_us{0};  ///< esp_timer_get_time() truncated to 32 bits: wraps every ~71 min, which
                     ///< the reader stitches from the monotonic sequence of records.
  uint32_t can_id{0};
  uint8_t data[MAX_FRAME_DATA_LEN]{};
  uint8_t dlc{0};
...
static_assert(sizeof(TapRecord) == 20, "TapRecord must stay a compact 20 bytes");

/// Bit positions in TapRecord::flags. Kept next to the struct so the writer and the ISR producer
...
template<uint16_t N, typename RecordT = FrameRecord> class ObserveRing {
 ...
 public:
  ...
  /// unread record.
  CAN_GATEWAY_CORE_INLINE bool push(const RecordT &record) {
...
/// How a signal marks "signal not available" (SNA), the DBC convention for a reading that does
/// not exist right now — sensor asleep, ECU not reporting, value not yet valid. Without this the
/// reserved raw value decodes as a confident number: a 10-bit state-of-charge field at SNA
/// publishes 1023, i.e. "102.3 %".
///
/// NONE is the default and is bit-for-bit the pre-SNA behavior: every raw value is a reading.
/// ALL_ONES is the dominant convention — in a real 165-message HV-battery DBC, all 377 SNA
/// entries in its VAL_ tables are all-ones for the signal's own bit width (a 2-bit signal uses
/// raw 3, a 10-bit signal raw 1023) and none is anything else. EXPLICIT is the escape hatch for
/// the DBC that reserves some other raw value.
enum class SnaMode : uint8_t { NONE = 0, ALL_ONES = 1, EXPLICIT = 2 };

/// The all-ones raw value of a `width`-bit field (1-32 bits). Width 32 is answered literally:
/// `1u << 32` is undefined behavior, not 0.
CAN_GATEWAY_CORE_INLINE uint32_t width_all_ones(uint8_t width) {
  return width >= 32 ? 0xFFFFFFFFu : ((1u << width) - 1u);
}

/// Is this field value the configured SNA marker? Deliberately tested on the RAW extracted value,
/// before sign_extend() and before any ESPHome `filters:`: a signed 8-bit signal at SNA is raw
/// 0xFF, which sign-extends to -1 — a perfectly plausible reading that no later stage can tell
/// apart from the real thing. The width is the signal's own (`bit_length`, or 8 * byte length),
/// so the same raw 0xFF is SNA in an 8-bit field and a plain 255 in a 16-bit one.
CAN_GATEWAY_CORE_INLINE bool raw_is_sna(uint32_t raw, uint8_t width, SnaMode mode, uint32_t sna_raw) {
  switch (mode) {
    case SnaMode::ALL_ONES:
      return raw == width_all_ones(width);
    case SnaMode::EXPLICIT:
      return raw == sna_raw;
    case SnaMode::NONE:
    default:
      return false;
