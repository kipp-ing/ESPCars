...
# A board can carry more than one LIN segment (the CAN_LIN_ESP32 reference board
# has two), and each segment needs its own UART. Instances are independent: the
# C++ side keeps no per-process state, and to_code() emits nothing global.
# The practical ceiling is enforced by _final_validate below — every instance
# must own a distinct uart_port, so a chip cannot host more segments than it has
# spare UARTs.
MULTI_CONF = True
...
# Keys of a validated pin that describe how it is *driven* rather than which
# physical pin it is. Everything else is identity: the number, plus — for a pin
# on an I/O expander — the key naming that expander, so pin 3 of two different
# expanders never looks like a collision. CONF_ID must be in here: the pin
# schema generates a unique id per pin, and leaving it in identity would make
# every pin distinct and quietly disable this check entirely.
_PIN_ROLE_KEYS = frozenset(
    {
        CONF_ID,
        CONF_MODE,
        CONF_INVERTED,
...
def _pin_identity(pin_conf):
    """Hashable identity of a pin, or None if it carries no comparable number.

    Built by *subtraction* so an unknown future key counts as identity: a new
    esphome pin option can then only ever cause a false collision report, which
    is loud, never a missed one, which would be silent.
    """
    if not isinstance(pin_conf, dict) or CONF_NUMBER not in pin_conf:
        return None
    return tuple(
        sorted((str(k), str(v)) for k, v in pin_conf.items() if k not in _PIN_ROLE_KEYS)
    )


def _pin_identities(config):
    """{pin identity: config key} for this instance's declared pins."""
    result = {}
    for key in (CONF_TX_PIN, CONF_RX_PIN, CONF_CS_PIN):
        if key not in config:
            continue
        identity = _pin_identity(config[key])
...
def _final_validate(config):
    """Cross-instance checks for multi-segment boards.

    Two linbus instances that share a UART or a GPIO break *both* channels, and
    the symptom on the bench is indistinguishable from dead hardware — silence
    on a bus that looks correctly wired. Catch it at validation instead.

    Runs once per instance and compares that instance against every other, so a
    conflicting pair is reported from whichever side the user reads first; the
    message always names the other instance.
    """
    instances = fv.full_config.get().get(DOMAIN) or []
    if not isinstance(instances, list):
        instances = [instances]

    my_id = config[CONF_ID]
    my_pins = _pin_identities(config)

    for other in instances:
        if other[CONF_ID] == my_id:
            continue

        if other[CONF_UART_PORT] == config[CONF_UART_PORT]:
            raise cv.Invalid(
                f"uart_port {config[CONF_UART_PORT]} is already used by linbus "
                f"'{other[CONF_ID]}'. Each LIN segment needs its own UART — "
                "sharing one silently breaks both.",
                path=[CONF_UART_PORT],
            )

        for identity, key in _pin_identities(other).items():
            if identity in my_pins:
                number = dict(identity).get(CONF_NUMBER, "?")
                raise cv.Invalid(
                    f"GPIO{number} is already used as {key} by linbus "
                    f"'{other[CONF_ID]}'.",
                    path=[my_pins[identity]],
                )
